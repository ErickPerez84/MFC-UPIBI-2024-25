...
void Adafruit_INA219::setCalibration_16V_400mA() {
  ...
  // 4. Choose an LSB between the min and max values
  //    (Preferrably a roundish number close to MinLSB)
  // CurrentLSB = 0.0000122 (50uA per bit)
...
/*!
 *  @brief  Configures the INA219 to measure up to 300mV on the bus, with a maximum current of 100uA.
 *          Each unit of current corresponds to 1uA, and each unit of power corresponds to 20nW.
 *          Counter overflow occurs at 100uA.
 *  @note   This configuration assumes a 10 ohm shunt resistor is used.
 */


void Adafruit_INA219::setCalibration_300mV_100uA() {
  // VBUS_MAX = 0.3V             (300mV maximum bus voltage)
  // VSHUNT_MAX = 0.3V           (300mV maximum shunt voltage)
  // RSHUNT = 10 ohms            (Resistor value)

  // 1. Determine max possible current
  // MaxPossible_I = VSHUNT_MAX / RSHUNT
  // MaxPossible_I = 0.3V / 10 ohms = 0.03A (30mA)

  // 2. Determine max expected current
  // MaxExpected_I = 100uA = 0.0001A

  // 3. Calculate possible range of LSBs
  // MinimumLSB = MaxExpected_I / 32767
  // MinimumLSB = 0.0001 / 32767 = 3.051e-9 A/bit (3.051nA/bit)
  // MaximumLSB = MaxExpected_I / 4096
  // MaximumLSB = 0.0001 / 4096 = 2.441e-8 A/bit (24.41nA/bit)

  // Choose a round value for CurrentLSB close to the MinimumLSB
  // CurrentLSB = 1e-6 (1µA per bit)

  // 4. Compute the calibration register
  // Cal = trunc(0.04096 / (CurrentLSB * RSHUNT))
  // Cal = trunc(0.04096 / (1e-6 * 10))
  // Cal = trunc(0.04096 / 0.00001) = 4096

  ina219_calValue = 4096;

  // 5. Calculate the power LSB
  // PowerLSB = 20 * CurrentLSB
  // PowerLSB = 20 * 1e-6 = 2e-5 W/bit (20nW/bit)

  // Set multipliers for current and power calculations
  ina219_currentDivider_mA = 1000; // Current LSB = 1µA per bit (1000/1 = 1000)
  ina219_powerMultiplier_mW = 0.02f; // Power LSB = 20nW per bit
  ...
  // Set Config register for 300mV bus voltage and shunt settings
  uint16_t config = INA219_CONFIG_BVOLTAGERANGE_16V | // Cambiar a 16V para mayor precisión
                  INA219_CONFIG_GAIN_1_40MV |       // Usar ganancia de 1 (40 mV rango shunt)
                  INA219_CONFIG_BADCRES_12BIT |    // Resolución de 12 bits para el bus
                  INA219_CONFIG_SADCRES_12BIT_1S_532US | // Resolución de 12 bits para el shunt
                  INA219_CONFIG_MODE_SANDBVOLT_CONTINUOUS; // Modo continuo
...
/*!
 *  @brief  Gets the bus voltage in millivolts (mV).
 *  @return The bus voltage in millivolts.
 */
float Adafruit_INA219::getBusVoltage_mV() {
  int16_t rawValue = getBusVoltage_raw(); // Get raw bus voltage value
  return rawValue * 0.001;               // Convert to millivolts (1 LSB = 1 mV)
}


/*!
 *  @brief  Gets the current in microamperes (uA).
 *  @return The current in microamperes.
 */
float Adafruit_INA219::getCurrent_uA() {
  int16_t rawValue = getCurrent_raw(); // Raw current value
  return (rawValue)-3.4; // Convert to µA and add offset
}


int16_t Adafruit_INA219::getRawShuntVoltage() {
  return getShuntVoltage_raw();
}

int16_t Adafruit_INA219::getRawBusVoltage() {
  return getBusVoltage_raw();
}

int16_t Adafruit_INA219::getRawCurrent() {
